...
/**
 * OffsetPaginationResponseZoneCarrierResponse model.
 */
class OffsetPaginationResponseZoneCarrierResponse implements \JsonSerializable
{
    private array $data;
    ...
    {
        return new self(
            array_map(fn($item) => ZoneCarrierResponse::fromArray($item), $data['data'] ?? []),
            $data['total_count'],
            $data['total_page'],
            $data['page'],
            $data['result_count'],
